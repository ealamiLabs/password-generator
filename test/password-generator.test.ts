...
describe('OfflinePasswordGenerator', () => {
  ...
  test('OfflinePasswordGenerator.getMinWordLength', () => {
    const generator = new OfflinePasswordGenerator([
      { word: 'test', length: 4, uniqueCharacters: 3 },
      { word: 'testtest', length: 8, uniqueCharacters: 3 },
    ]);
    expect(generator.getMinWordLength()).toBe(4);
  });

  test('OfflinePasswordGenerator.getMaxWordLength', () => {
    const generator = new OfflinePasswordGenerator([
      { word: 'test', length: 4, uniqueCharacters: 3 },
      { word: 'testtest', length: 8, uniqueCharacters: 3 },
    ]);
    expect(generator.getMaxWordLength()).toBe(8);
  });

  test('OfflinePasswordGenerator.getRandomWord', () => {
    const generator = new OfflinePasswordGenerator([
      { word: 'test', length: 4, uniqueCharacters: 3 },
      { word: 'testtest', length: 8, uniqueCharacters: 3 },
    ]);
    expect(generator.getRandomWord(4).word).toBe('test');

    generator.setDictionary([]);
    expect(() => generator.getRandomWord(4)).toThrow(
      'No words with the given length'
    );
  });
